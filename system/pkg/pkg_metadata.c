/****************************************************************************
 * apps/system/pkg/pkg_metadata.c
 *
                                       ...
                                       FAR struct pkg_manifest_s *manifest)
{
  ...
  if (value == NULL ||
      (ret = pkg_copy_string(manifest->name, sizeof(manifest->name), value)) < 0)
    {
      return -EINVAL;
    }
  ...
  value = cJSON_GetStringValue(field);
  if (value == NULL ||
      (ret = pkg_copy_string(manifest->arch, sizeof(manifest->arch), value)) < 0)
    {
      return -EINVAL;
    }
  ...
  value = cJSON_GetStringValue(field);
  if (value == NULL ||
      (ret = pkg_copy_string(manifest->sha256, sizeof(manifest->sha256), value)) < 0)
    {
      return -EINVAL;
    }
...
static int pkg_metadata_parse_installed_entry(FAR cJSON *item,
                                              FAR struct pkg_installed_entry_s *entry)
{
  FAR cJSON *field;
  FAR const char *value;
  ...
  if (value != NULL)
    {
      ret = pkg_copy_string(entry->previous, sizeof(entry->previous), value);
      if (ret < 0)
        {
          return ret;
        }
  ...
  value = cJSON_GetStringValue(field);
  if (value == NULL ||
      (ret = pkg_copy_string(entry->compat, sizeof(entry->compat), value)) < 0)
    {
      return -EINVAL;
    }
...
static FAR cJSON *pkg_metadata_manifest_to_json(FAR const struct pkg_manifest_s *manifest)
{
  FAR cJSON *root;
  ...
  cJSON_AddStringToObject(root, "type", pkg_manifest_type_str(manifest->type));
  return root;
}
  ...
  for (i = 0; i < db->count; i++)
    {
      ...
      fprintf(stream,
              ...
              pkg_manifest_type_str(entry->type), entry->arch, entry->compat);
