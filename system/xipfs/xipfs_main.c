                          ...
                          FAR struct xipfs_survey_s *s)
{
      ...
      for (f = 0; f < s->nfiles; f++)
        {
          ...
          printf("  %-*s %6lu %7lu %8lu %4lu\n", width, e->name,
                 (unsigned long)e->start, (unsigned long)e->nblocks,
                 (unsigned long)e->size, (unsigned long)e->pincount);
        }
    }
  ...
  printf("\n  used %lu, free %lu, largest free run %lu blocks (%lu KB)\n",
         (unsigned long)s->used, (unsigned long)s->freeblocks,
         (unsigned long)s->largestrun,
         (unsigned long)(s->largestrun * (uint64_t)s->blocksize / 1024));
  printf("  %lu free run%s, fragmentation %lu%%\n",
         (unsigned long)s->nruns, s->nruns == 1 ? "" : "s",
         (unsigned long)frag);

  if (s->pinned > 0)
    {
      printf("  %lu block%s pinned and cannot be relocated\n",
             (unsigned long)s->pinned, s->pinned == 1 ? "" : "s");
    }
  ...
  if (frag == 0)
    ...
    {
      printf("  compacting could raise the largest run to %lu blocks"
             " (%lu KB)\n",
             (unsigned long)s->freeblocks,
             (unsigned long)(s->freeblocks * (uint64_t)s->blocksize / 1024));
    }
}
                      ...
                      uint32_t max_ms)
{
  ...
  printf("  %s: moved %lu extent%s, reclaimed %lu block%s, %lu pinned\n",
         reason_str(arg.result.reason),
         (unsigned long)arg.result.extents_moved,
