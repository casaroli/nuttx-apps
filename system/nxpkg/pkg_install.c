...
static bool pkg_install_has_version(FAR const struct pkg_installed_entry_s *entry,
                                   ...
                                   FAR const char *version)
{
  ...
  ret = snprintf(entry->versions[entry->version_count],
                 sizeof(entry->versions[entry->version_count]), "%s", version);
  if (ret < 0)
    {
      return ret;
    }
                                        ...
                                        FAR const struct pkg_manifest_s *manifest)
{
  ...
  ret = snprintf(entry->compat, sizeof(entry->compat), "%s", manifest->compat);
...
static int pkg_install_write_pointers(FAR const struct pkg_installed_db_s *db,
                                      FAR const struct pkg_manifest_s *manifest)
{
  FAR struct pkg_installed_entry_s *entry;
...
int pkg_install(FAR const char *name)
{
  ...
  ret = pkg_store_format_payload_path(payload, sizeof(payload), manifest->name,
                                      manifest->version, manifest->artifact);
  if (ret < 0)
    {
      goto errout;
    }
