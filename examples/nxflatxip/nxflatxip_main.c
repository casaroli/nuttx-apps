...
static int stage_module(void)
{
  ...
  fd = open(MODPATH, O_WRONLY | O_CREAT | O_TRUNC, 0755);
  if (fd < 0)
    {
...
static int bench_main(void)
{
  ...
  fd = open(MOUNTPT "/benchw", O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (fd >= 0)
    {
      write(fd, "x", 1);
      close(fd);
  ...
  for (i = 0; i < dblocks; i += DF_MAPCOLS)
    {
      ...
      row[n] = '\0';
      syslog(LOG_INFO, "    %3lu |%s|\n", (unsigned long)i, row);
    }

  syslog(LOG_INFO, "    free %lu blocks, largest contiguous run %lu\n",
...
static int df_try_alloc(uint32_t nblocks, uint32_t erasesize)
{
  ...
  fd = open(MOUNTPT "/bigalloc", O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (fd < 0)
    {
      return -errno;
    }
...
static int defrag_main(void)
{
  ...
  syslog(LOG_INFO, "\n[3] requesting a %lu-block (%lu KB) contiguous file\n",
         (unsigned long)want, (unsigned long)(want * erasesize / 1024));
  ...
  syslog(LOG_INFO, "    extents relocated: %lu\n",
         (unsigned long)res.extents_moved);
  syslog(LOG_INFO, "    blocks reclaimed : %lu\n",
         (unsigned long)res.blocks_reclaimed);
  syslog(LOG_INFO, "    largest free run : %lu bytes (%lu blocks)\n",
         (unsigned long)res.largest_free_run,
         (unsigned long)(res.largest_free_run / erasesize));
