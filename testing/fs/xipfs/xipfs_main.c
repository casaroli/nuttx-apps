                      ...
                      FAR struct xipfs_defrag_result_s *result)
{
  ...
  fd = open(MOUNTPT, O_RDONLY | O_DIRECTORY);
  if (fd < 0)
    {
      return -errno;
    }
...
static void test_multimap(void)
{
  ...
  for (i = 0; i < 3; i++)
    {
      fd[i] = open(PATH("multi.bin"), O_RDONLY);
...
static int holder_task(int argc, FAR char *argv[])
{
  ...
  fd = open(PATH("shared.bin"), O_RDONLY);
  if (fd < 0)
    {
      return EXIT_FAILURE;
    }
...
static int leaky_mapper(int argc, FAR char *argv[])
{
  ...
  fd = open(PATH("leak.bin"), O_RDONLY);
  if (fd < 0)
    {
      return EXIT_FAILURE;
    }
...
static void test_dirs(void)
{
  ...
  fd = open(PATH("nodir/x"), O_WRONLY | O_CREAT | O_TRUNC, 0644);
  ...
  fd = open(PATH("top.bin/deeper"), O_WRONLY | O_CREAT | O_TRUNC, 0644);
  ...
  fd = open(PATH("bin"), O_WRONLY | O_CREAT, 0644);
...
static void test_strict(void)
{
  ...
  fd = open(PATH("strict.bin"), O_RDONLY);
  if (fd < 0)
    {
...
static void test_writeonce(void)
{
  ...
  fd = open(PATH("once.bin"), O_WRONLY);
  ...
  fd = open(PATH("once.bin"), O_WRONLY | O_APPEND);
  ...
  fd = open(PATH("hole.bin"), O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (fd >= 0)
    {
      ftruncate(fd, len);
