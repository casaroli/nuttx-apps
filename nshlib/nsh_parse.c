                       ...
                       FAR const char *redirfile_out, int oflags)
{
  ...
  ret = nsh_builtin(vtbl, argv[0], argv, redirfile_in, redirfile_out, oflags);
  if (ret >= 0)
    {
  ...
  if (*pbegin == '<')
    {
      ...
      if (*(pbegin + 1) == '<')
        {
          *saveptr = pbegin + 2;
          argument = (FAR char *)g_redirect_in2;
        }
      else
        {
          *saveptr = pbegin + 1;
    ...
    /* Does the token begin with '|' -- pipe */

    if (*pbegin == '|')
    {
      *saveptr = pbegin + 1;
      argument = (FAR char *)g_pipe;
    }
  ...
  for (argc = 1; argc < MAX_ARGV_ENTRIES - 1;)
    {
      ...
      if (isenvvar != 0)
      ...
      else if (!strncmp(argv[argc], g_redirect_out2, strlen(g_redirect_out2)))
        {
          char *arg;
      ...
      else if (!strncmp(argv[argc], g_redirect_out1, strlen(g_redirect_out1)))
        {
          char *arg;
      ...
      else if (!strncmp(argv[argc], g_redirect_in2, strlen(g_redirect_in2)))
        {
          ...
          if (argv[argc][strlen(g_redirect_in2)])
            {
              arg = &argv[argc][strlen(g_redirect_in2)];
            }
          else
            {
              arg = nsh_argument(vtbl, &saveptr, &memlist, NULL, &isenvvar);
          ...
          // TODO: implement
          nsh_error(vtbl, "Redirect type << NOT SUPPORTED (yet)\n");
          ret = ERROR;
          goto dynlist_free;

        }
      else if (!strncmp(argv[argc], g_redirect_in1, strlen(g_redirect_in1)))
        {
          char *arg;
          if (argv[argc][strlen(g_redirect_in1)])
            {
              arg = &argv[argc][strlen(g_redirect_in1)];
            }
          else
            {
              arg = nsh_argument(vtbl, &saveptr, &memlist, NULL, &isenvvar);
